...
const load = async () => {
  ...
  try {
    const [listRes, manualRes, statsRes] = await Promise.all([
      getReviewList(filter.value || undefined, page.value, size.value),
      getManualReviews(1, 5),
      getReviewStats()
    ])
    reviews.value = listRes.data.data.content
    total.value = listRes.data.data.totalElements
    manualList.value = manualRes.data.data.content
    manualTotal.value = manualRes.data.data.totalElements
