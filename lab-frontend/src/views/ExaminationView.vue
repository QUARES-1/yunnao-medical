...
import { reviewExamination, updateResult } from '@/api/lab'
...
const loadData = async () => {
  loading.value = true
  try {
    const pageData = await labStore.loadExaminations(props.mode, page.value, size.value)
    list.value = props.mode === '待检查' ? labStore.pendingExaminations : labStore.completedReports
...
const openDetail = async (row: Examination) => {
  current.value = await labStore.selectExamination(row.id)
  drawer.value = true
}
