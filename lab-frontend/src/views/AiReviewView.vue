    ...
    <div class="review-grid">
      <section class="content-card">
        ...
        <el-table :data="reviews" v-loading="loading" class="pharmacy-table">
          ...
          <el-table-column label="检查项目" min-width="150">
            <template #default="{row}"><strong class="item-name">{{ row.itemName || '未关联项目' }}</strong></template>
          </el-table-column>
      ...
      <section class="content-card manual-panel">
        ...
        <article v-for="item in manualList" :key="item.id" class="manual-card" @click="open(item)">
          ...
          <div>
            <strong>{{ item.itemName || '未关联项目' }}</strong>
            <p>EX{{ String(item.examinationId).padStart(6, '0') }} · {{ firstWarning(item) || 'AI建议人工复核该报告。' }}</p>
          </div>
...
.item-name{font-weight:700;color:#073f4d}
.review-op{display:flex;align-items:center;gap:10px;flex-wrap:wrap;justify-content:flex-start}
.review-op .el-button{margin-left:0!important;white-space:nowrap}
.review-op .el-button--small{padding:6px 14px;min-width:76px}
</style>
