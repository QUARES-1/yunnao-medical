  ...
  <view class="page safe-bottom">
    ...
    <scroll-view class="messages" scroll-y :scroll-into-view="lastMessageId">
      <view v-for="(msg, index) in messages" :id="`msg-${index}`" :key="index" class="message" :class="msg.role">
        <view class="bubble">
          <text>{{ msg.content }}<text v-if="msg.streaming" class="stream-cursor">▋</text></text>
      ...
      <view v-if="loading" id="loading" class="message ai"><view class="bubble loading">正在连接 AI 服务并分析问题...</view></view>
    </scroll-view>

    <view class="composer">
      <input v-model="question" class="input" confirm-type="send" placeholder="输入你想咨询的问题" @confirm="send" />
      <button class="send" :loading="loading || streaming" :disabled="loading || streaming" @tap="send">发送</button>
    </view>
  </view>
</template>
...
const aiStore = usePatientAiStore()
const { messages, loading, streaming } = storeToRefs(aiStore)
const question = ref('')
...
function scrollBottom() {
  nextTick(() => {
    lastMessageId.value = loading.value ? 'loading' : `msg-${messages.value.length - 1}`
  })
}
...
watch(() => messages.value.map(item => item.content.length).join(','), scrollBottom)
onLoad((query) => {
...
.stream-cursor{display:inline-block;margin-left:3rpx;color:#0b918d;animation:blink .75s steps(2,start) infinite}@keyframes blink{50%{opacity:0}}
</style>
