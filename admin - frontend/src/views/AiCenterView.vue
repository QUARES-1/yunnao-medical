<template>
  <div class="ai-page" v-loading="loading">
    <section class="ai-hero">
      ...
      <el-button type="primary" size="large" @click="generateReport"><el-icon><MagicStick /></el-icon>生成运营报告</el-button>
    </section>
...
import { addKnowledge, generateAiReport, getAiChatLogs, getAiOverview, getAiReportList, getQualityCheckList, startQualityCheck } from '@/api/ai'
import type { AiChatLog, AiKnowledge, AiOperationReport, QualityCheckRecord } from '@/types'
...
async function loadAll() {
  ...
  try {
    const [overviewRes, reportRes, checkRes, logRes] = await Promise.all([
      ...
      getAiReportList({ reportType: reportType.value || undefined, page: 1, size: 6 }),
...
async function loadReports() {
  const res = await getAiReportList({ reportType: reportType.value || undefined, page: 1, size: 6 })
  reports.value = res.data.data.content || []
}
async function generateReport() {
  loading.value = true
  try {
    await generateAiReport({ reportType: 'daily' })
    ElMessage.success('AI运营报告已生成')
    await loadAll()
  } finally { loading.value = false }
}
